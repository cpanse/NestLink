...
aa_pool_x7.post <- c(rep('WR', 20), rep('WLTVR', 20), rep('WQEGGR', 20), rep('WLR', 20), rep('WQSR', 20))
...
## ssrc calc prediction vs. measured rtinseconds
...
ggplot(WU, aes(x = RTINSECONDS, y = moverz * pep_exp_z)) +
  geom_point(aes(alpha = pep_score, colour = datfilename)) +
  facet_wrap(~ datfilename * suffix, ncol=5) 
```

```{r fig.retina=3, fig.width=12, fig.height=4}
ggplot(WU, aes(x = ssrc, y = moverz * pep_exp_z)) +
  geom_point(aes(alpha=pep_score, colour = datfilename)) +
  facet_wrap(~ datfilename * suffix,  ncol = 5)
```
...
## Trellis plots in dep. from the Mascot Ion Score cut-off
```{r fig.retina=3, fig.height=12}
WU$cutoff <- 25
WU40 <- WU[WU$pep_score >= 40,]
WU60 <- WU[WU$pep_score >= 60,]
WU40$cutoff <- 40
WU60$cutoff  <- 60

ggplot(rbind(WU, WU40, WU60), aes(x = RTINSECONDS, y = moverz * pep_exp_z)) +
  geom_point(aes(colour = suffix),  size = 3.0, alpha = 0.1) +
  facet_wrap(~cutoff, ncol = 1)

ggplot(rbind(WU, WU40, WU60), aes(x = ssrc, y = moverz * pep_exp_z)) +
  geom_point(aes(colour = suffix),  size = 3.0, alpha = 0.1) +
  facet_wrap(~cutoff, ncol=1)
