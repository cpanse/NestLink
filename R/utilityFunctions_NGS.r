...
getRowNameMax = function(X){
  return(names(sort(X,decreasing=T))[1])
}
...
filterByReadLengthMedian = function(myReads,relMinLength=0.95,relMaxLength=1.05){
  medianWidth = median(width(myReads))
  filteredReads = myReads[which(width(myReads) > medianWidth*relMinLength & width(myReads) < medianWidth*relMaxLength)]
  return(filteredReads)
}

filterReadsWithNs = function(mySeq){
  readsWithNs=unique(c(which(grepl('N',sapply(mySeq[['seqFC']],toString))),which(grepl('N',sapply(mySeq[['seqNB']],toString)))))
...
filterForInFrame = function(mySeq,minFlycodeLength=33,minNanobodyLength=365){
  width_seqFC = width(mySeq[['seqFC']])
  width_seqNB = width(mySeq[['seqNB']])
  mySeq[['seqFC']] = mySeq[['seqFC']][(width_seqFC%%3==0 & width_seqFC>=minFlycodeLength & width_seqNB%%3==0 & width_seqNB>=minNanobodyLength)]
  mySeq[['seqNB']] = mySeq[['seqNB']][(width_seqFC%%3==0 & width_seqFC>=minFlycodeLength & width_seqNB%%3==0 & width_seqNB>=minNanobodyLength)]
  return(mySeq)
}

filterForStopCodons = function(mySeqAS){
  readsWithStops=unique(c(which(grepl('\\*',sapply(mySeqAS[['seqAS_FC']],toString))),which(grepl('\\*',sapply(mySeqAS[['seqAS_NB']],toString)))))
...
filterForStartEndPattern = function(mySeqAS,myFCPattern='^GS.*R$'){
  readsWithCorrectFlyCodeEnds = which(grepl(myFCPattern,sapply(mySeqAS[['seqAS_FC']],toString)))
...
filterForMinFlycodeFrequency = function(mySeqAS,minFreq,file,knownFC=c()){
  ...
  tmp = sort(table(FCReads),decreasing=T)
  tmp = tmp[tmp>=minFreq]
  top_FC = data.frame(Sequence=names(tmp),Frequency=tmp,stringsAsFactors = F)
  if(length(knownFC)>0){
    top_FC[['Name']] = '-'
  ...
  write.table(top_FC,top_FCFile,sep='\t',quote=F,row.names=F)
  mySeqAS[['seqAS_FC']] = AAStringSet(names(tmp))
  names(mySeqAS[['seqAS_FC']]) = paste('read',seq(1,length(mySeqAS[['seqAS_FC']]),1),sep='_')
  return(mySeqAS)
}

hit2 = function(data){
  secondBest = sort(data,decreasing=T)[2]
  return(secondBest)
}

getConsensusNBs = function(uniqFCReads,bigTable){
  uniq_FC2NB = data.frame(Flycode=uniqFCReads, NanoBody='', consensusScore=0, missmatchEvents=0, 
                          relBestHitFreq = 0, stringsAsFactors = F)
  for (j in 1:length(uniqFCReads)){
    ...
    consensusScore = mean((hit1-hit2)/sum)
    ...
    if(relBestHitFreq < 0.5){
       ...
       write.table(c(NBperFC[['NanoBody']],mismatchData),fileName,sep='', quote=F, row.names = F, col.names=F)
       #pheatmap::pheatmap(cMatrix,cluster_rows = F,color=gray.colors(max(cMatrix)), cluster_cols = F,,fileName=fileName)
    }
    
    uniq_FC2NB[['NanoBody']][j] = paste(apply(cMatrix,2,getRowNameMax),collapse='')
...
findTopNB = function(seqAS_NB,n=100,knownNB){
  top_NB = head(sort(table(sapply(seqAS_NB,toString)),decreasing = T),n=n)
  top_NB = data.frame(Sequence=names(top_NB),Frequency=top_NB,stringsAsFactors = F)
  top_NB[['Name']] = '-'
