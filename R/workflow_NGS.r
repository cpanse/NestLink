...
runNGSAnalysis <- function(file, param){
  ...
  write.table(bigTable, bigTableFileName, sep='\t',row.names = FALSE, quote = FALSE)
  
  keepFCs <- rownames(bigTable)[bigTable$relBestHitFreq >= param[['minRelBestHitFreq']] 
                                & bigTable$consensusScore >= param[['minConsensusScore']]] 
  ...
  write.table(stats, statsFileName, sep = '\t', quote = FALSE, row.names = FALSE)
  
  top100_NBFile <- basename(sub('.fastq.gz', '_TopNB.txt', file))
  top100_NB <- .findTopNB(mySeqAS_CS[['seqAS_NB']], n = 100, knownNB)
  write.table(top100_NB, top100_NBFile, sep = '\t', quote = FALSE, row.names = FALSE)
  
  #BigTable: Nanobody2Flycode
  uniqNB <- unique(bigTable$NanoBody)
  uniqNB_Summary <- data.frame(NB = uniqNB, stringsAsFactors = FALSE)
  ...
  for (j in 1:length(uniqNB)){
    ...
    uniqNB_Summary <- uniqNB_Summary[order(uniqNB_Summary$NB_Name, decreasing = TRUE), ]
  }
  write.table(uniqNB_Summary, basename(sub('.fastq.gz', '_uniqNB2FC.txt', file)), 
              sep = '\t', row.names = FALSE, quote = FALSE)
  
  uniqFC <- unique(bigTable$Flycode)
  uniqFC_Summary <- data.frame(FC = uniqFC, stringsAsFactors = FALSE)
