...
vignette: |
  %\VignetteIndexEntry{FASTA p1875 db10 - Postprocessing / Summary}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
  
date: '2017-11-02, 2017-11-03, 2017-11-04, 2017-11-06, 2017-11-08, 2018-01-30, 2018-02-01'
---
...
- Results of using the ESP from []() TODO

## Import Data into R

The following code snippet has been executed before and is only extracting the meta data (no MS2).
...
```{r fig.retina=3}
ESP <- rbind(NestLink:::.getFC(), NestLink:::.getNB())
...
ESP <- rbind(NestLink:::.getFC(), NestLink:::NB.unambiguous(NestLink:::.getNB()))
p <- ggplot(ESP, aes(x = ESP_Prediction, fill = cond)) +
  geom_histogram(bins = 50, alpha = 0.4, position="identity") +
  labs(x = "detectability in LC-MS (ESP prediction)") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
ESP <- rbind(NestLink:::.getFC(), NestLink:::NB.unique(NestLink:::.getNB()))
p <- ggplot(ESP, aes(x = ESP_Prediction, fill = cond)) +
  geom_histogram(bins = 50, alpha = 0.4, position="identity") +
  labs(x = "detectability in LC-MS (ESP prediction)") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
ESP <- rbind(NestLink:::.getNB(), NestLink:::NB.unambiguous(NestLink:::.getNB()), NestLink:::NB.unique(.getNB()))
