...
date: '2017-11-02, 2017-11-03, 2017-11-04, 2017-11-06'
...
ggplot(SS, aes(x = moverz * chargeInt, fill = (query_charge), colour = (query_charge))) +
...
```{r}
SS$postfix <- substr(SS$pep_seq, 10, 100)

ggplot(SS, aes(x = moverz * chargeInt, fill = postfix, colour = postfix)) +
...
```{r}
ggplot(SS, aes(x = moverz * chargeInt, fill = postfix)) +
...
```{r}
ggplot(SS, aes(x = pep_score, fill = query_charge, colour = query_charge)) +
...
WU160118.pep_seq.freq.post <- table(unlist(substr(WU160118.pep_seq, 10, 100)))
WU160118.freq.post <- round(WU160118.pep_seq.freq.post / sum(WU160118.pep_seq.freq.post) *100)
```
...
AAfreq3 <- data.frame(aa=WU160118.freq)
names(AAfreq3) <- c('AA', 'freq')
AAfreq3$cond <- "WU160118"
...
AAfreq3.post <- data.frame(aa=WU160118.freq.post)
names(AAfreq3.post) <- c('AA', 'freq')
AAfreq3.post$cond <- "WU160118"
...
ggplot(data=AAfreq.post, aes(x=AA, y=freq, fill=cond)) + 
  geom_bar(stat="identity", position="dodge") + 
  labs(title = "postfix frequency plot") +
  theme_light()
```
