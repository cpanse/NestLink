...
```{r fig.cap='measured log2 ratios were plotted. red represents the sd and blue points the mean.', fig.retina=3}
P<-na.omit(P)
xyplot(log2ratios ~  FCset_ng,
       data = P,
       panel = function(x,y){
         panel.abline(h=0, col='red')
          panel.xyplot(x,y, pch=16, col=rgb(0.5,0.5,0.5,alpha = 0.4))
          panel.points(x, mean(y), pch=16)
...
FCstatistics <- function(P){
 ...
 rv <- data.frame(FCset_ng=P.mean$FCset_ng, mean=P.mean$mean, sd=P.sd$sd, cv=P.sd$sd/P.mean$mean, length=P.length$length)
 rv
}
