...
SIM <- P.replicate
SIM$type <- "bio"
P.replicate.coli$type <- "tech"
P.replicate.smeg$type <- "tech"

xyplot(cv ~ i | ConcGr * type, 
       data=do.call('rbind', list(SIM,P.replicate.coli,P.replicate.smeg)),
       panel = function(x,y,...){
         panel.abline(h=10, col='red')
         panel.xyplot(x,y,...)
         xy.mean <- (aggregate(y, by=list(x), FUN=median, na.rm = T))
           
          panel.points( xy.mean[,1], xy.mean[,2],pch=16)
         #points(x[1], median(y))
         #panel.loess(...)
       },
       strip = strip.custom(strip.names = TRUE, strip.levels = TRUE),
