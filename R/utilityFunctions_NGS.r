...
filterForMinFlycodeFrequency <- function(mySeqAS, minFreq, file, knownFC = c()){
  ...
  tmp <- tmp[tmp >= minFreq]
  top_FC <- data.frame(Sequence = names(tmp), Frequency = tmp, stringsAsFactors = FALSE)
  if(length(knownFC) > 0){
    top_FC[['Name']] <- '-'
...
getConsensusNBs <- function(uniqFCReads, bigTable){
  ...
  for (j in 1:length(uniqFCReads)){
    ...
    if(relBestHitFreq < 0.5){
       ...
       write.table(c(NBperFC[['NanoBody']], mismatchData), fileName, sep = '', quote = FALSE, row.names = FALSE, col.names = FALSE)
...
findTopNB <- function(seqAS_NB, n = 100, knownNB){
  top_NB <- head(sort(table(sapply(seqAS_NB,toString)),decreasing = TRUE),n = n)
  top_NB <- data.frame(Sequence=names(top_NB),Frequency=top_NB,stringsAsFactors = FALSE)
  top_NB[['Name']] <- '-'
