...
runNGSAnalysis <- function(file, param){
  ...
  names(mySeq[['seqFC']]) <- paste('read', 
                                   seq(1, length(mySeq[['seqFC']]),1),sep='_')
  ...
  bigTable <- data.frame(ID = names(mySeqAS[['seqAS_FC']]), 
                         Flycode = vapply(mySeqAS[['seqAS_FC']],
                                          toString, c(FC='')),
                         NanoBody = vapply(mySeqAS[['seqAS_NB']], 
                                           toString, c(NB='')), 
  ...
  write.table(bigTable, bigTableFileName, sep='\t', 
  ...
  for (j in seq_len(length(uniqNB))){
    ...
    if(uniqNB[j] %in% knownNB){
      uniqNB_Summary[['NB_Name']][j] <- names(knownNB)[knownNB==uniqNB[j]]
    }
    uniqNB_Summary <- uniqNB_Summary[order(uniqNB_Summary$NB_Name,
                                           decreasing=TRUE), ]
  }
  write.table(uniqNB_Summary, 
